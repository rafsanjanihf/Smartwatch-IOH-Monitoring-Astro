...
const InfoCard = ({ label, value, unit, className = "" }: InfoCardProps) => (
  <div className={`p-3 lg:p-4 bg-gray-50 rounded-lg ${className}`}>
    ...
    >
      {value}
      {unit && (
        <span className="text-xs lg:text-sm text-gray-500 ml-1">{unit}</span>
      )}
    </div>
  </div>
);
...
export default function SleepChart({
  ...
  useEffect(() => {
    ...
    const handleDateRangeChange = (e: CustomEvent) => {
      ...
      const deviceId = document
        .querySelector("[data-device-list] .bg-blue-50")
    ...
    document.addEventListener(
      "shift-filter-change",
      handleShiftChange as EventListener
    );
    document.addEventListener("sleep-data-update", ((
      e: CustomEvent<SleepData[]>
    ...
    return () => {
      ...
      document.removeEventListener("sleep-data-update", ((
        e: CustomEvent<SleepData[]>
      ) => setCurrentData(e.detail)) as EventListener);
    };
  }, [selectedDeviceId]);
  ...
  useEffect(() => {
    ...
    const sortedData = [...currentData]
      .sort(
        (a, b) =>
          (a.sleepMotion[0]?.startTime || 0) -
          (b.sleepMotion[0]?.startTime || 0)
      )
      .flatMap((sleep) => {
        let sleepMotions =
          sleep.sleepMotion?.sort((a, b) => a.startTime - b.startTime) || [];

        return sleepMotions.map((motion) => {
          const duration = (motion.endTime - motion.startTime) / 1000;
          const stage = Object.values(SLEEP_STAGES).find(
            (s) => s.value === motion.value
          );
    ...
    const activeStagesList = Object.values(SLEEP_STAGES).filter((stage) =>
      activeStages.has(stage.value)
    );
    ...
    const option: echarts.EChartsOption = {
      ...
      tooltip: {
        ...
        formatter: (params: any) => {
          ...
          const stage =
            Object.values(SLEEP_STAGES).find((s) => s.value === stageValue)
              ?.name || "Unknown";
          ...
          let html = `<div style="font-size: 14px;">
            ...
            <p style="margin: 5px 0;"><strong>Duration:</strong> ${Math.round(
              duration
            )} minutes</p>
      ...
      series: [
        {
          ...
          renderItem: (
            ...
            const stageIndex = activeStagesList.findIndex(
  ...
  const StageCard = ({
    ...
    stage: keyof typeof SLEEP_STAGES;
    time: number;
  }) => (
  ...
  return (
    <div className={`bg-white rounded-lg p-4 lg:p-6 ${className}`}>
        ...
        <div className="text-left mb-3 lg:mb-4">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-2 lg:gap-4">
            <h5 className="text-lg lg:text-2xl font-bold text-gray-900">
              ...
              <span
                className={`inline-flex items-center px-2.5 py-1 ml-3 text-xs font-medium rounded-full border ${getShiftBadgeColor(
                  currentShiftType
                )}`}
              >
      ...
      <div
        ref={chartRef}
        className="w-full"
        style={{ height: "min(200px, 35vh)" }}
      />

      <div className="mt-6 lg:mt-8">
        <div className="flex justify-between items-center mb-3 lg:mb-4">
          <h5 className="text-lg lg:text-xl font-bold text-gray-900">
            Additional Information
          </h5>
          <Switch.Group>
            <div className="flex items-center">
              <Switch.Label className="mr-2 lg:mr-3 text-xs lg:text-sm text-gray-600">
                List View
              </Switch.Label>
              <Switch
                checked={isListView}
        ...
        {currentData?.[0] &&
          (isListView ? (
            <div className="space-y-2">
              {Object.entries({
                "Sleep Time": {
                  value: formatDuration(sleepTimes.totalTime),
                  unit: "",
                },
                "Sleep Quality": {
                  value: (currentData[0].sleepQuality * 100).toFixed(2),
                  unit: "%",
                },
                "Heart Rate": {
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-4 gap-3 lg:gap-4">
              <InfoCard
                ...
                value={formatDuration(currentData[0].sleepTotalTime)}
              />
              <InfoCard
                label="Sleep Quality"
                value={(currentData[0].sleepQuality * 100).toFixed(2)}
